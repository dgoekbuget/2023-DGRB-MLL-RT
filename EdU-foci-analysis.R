#This script is used to determine S subphase and EdU foci location relative to nucleus center using Opera phenix output
...
#Read in Opera Phenix data for nuclei and spots
...
#Define column numbers for DNA sum, Log EdU Sum
colnames(res)

dna.sum <- 33
log.edu.sum <- 43

#Subset and reformat raw data
dat <- res[,c(16,dna.sum,log.edu.sum,45:46)]
colnames(dat) <- c("area","dna_sum","log_edu_sum","treatment","time")

#Downsample each timepoint and group for EdU plots
...
#Determine sub-S-phase quartiles
...
#Plot and color-code selected sub S population
...
#Gate non-downsampled data based on above sub-Sphase gates
res$subS <- NA
...
#Subset nuclei data and spot data in preparation to merging
...
#Merge spot data based on which nucleus they associate with
