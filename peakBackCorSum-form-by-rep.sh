...
if find /blellochlab/data1/Boileau/data/cutandtag/naivetoform/*/03_peak_calling/05_consensus_peaks -name *orm*"$mark"*.consensus.peaks.bed | while read filename; do cat "$filename" | bedtools subtract -a "$genome" -b stdin | awk -v OFS="\t" '{print NR,$1,$2,$3,"-"}' > "$outdir"/"$mark".temp.saf; done; then
...
bamfiles=($(find /blellochlab/data1/Boileau/data/cutandtag/naivetoform/all_bam_files -name *orm*"$mark"*.target.markdup.bam))
...
plength=($(find /blellochlab/data1/Boileau/data/cutandtag/naivetoform/*/03_peak_calling/05_consensus_peaks -name *orm*"$mark"*.consensus.peaks.bed | while read filename; do cat "$filename" | awk -v OFS='\t' '$10 != 1 {print}' | awk '{print int($3-$2)}'; done))
...
if find /blellochlab/data1/Boileau/data/cutandtag/naivetoform/*/03_peak_calling/05_consensus_peaks -name *orm*"$mark"*.consensus.peaks.bed | while read filename; do cat "$filename" | awk -v OFS='\t' '$10 != 1 {print}'| awk -v OFS='\t' -F'[\t,]' '{print $1,$2,$3,$8,$9}' | paste - <(echo ${plength[@]}) | awk -v i="$background" -v OFS='\t' '{print $1,$2,$3,$4-i*$6,$5-i*$6}' | bedtools map -a $reference -b stdin -c 4,5 -o sum -F 0.5 -null 0 > "$workdir"/"$mark".RTdomain.backgroundCorSum.bed; done
